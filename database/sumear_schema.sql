...
CREATE TABLE projects (
    ...
    ai_brief              TEXT,                -- cache synthèse IA (voir page projet)
    ai_brief_fingerprint  TEXT,                -- SHA-256 liste produits (ids+noms+prix…) ; si changement → régénérer
    created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at  TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- Plan / Stripe : modifiables uniquement avec la clé service_role (webhooks, scripts serveur)
...
-- Fonction pour incrémenter l'usage (appelée par le backend Next.js via supabase.rpc)
...
-- Agrégat quotas (aligné sur lib/config/plans.ts — garder les chiffres sync)
-- Non utilisé par l’app Next aujourd’hui (checkQuota TS) ; utile pour RPC / futur GET /api/usage.
...
DECLARE
    ...
    v_year        INTEGER;
    v_month       INTEGER;
    -- Limites free (complete / pro = illimité → -1 côté JSON)
...
BEGIN
    ...
    RETURN jsonb_build_object(
        ...
        -- Rétrocompat minimal (anciens champs)
